...
sum_museum %>%
  ...
  labs(title = "Average Sell Price in Each Month",
       x = "Months",
       y = "Sell Price",
       color=NULL)
...
sum_museum %>% 
  ...
  theme_classic()+
  theme(legend.position="none")+
  labs(title = "Average Sell Price in Each Hour",
       x = "Hours",
       y = "Sell Price",
       color=NULL)
```
...
museum %>% 
  ...
  ggplot(aes(x=sell, color=where_how))+
  geom_density()+
  facet_wrap(~where_how, ncol=4, scales="free_y")+
  theme_classic()+
  theme(legend.position="none")+
